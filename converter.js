...
async function main() {
  ...
  if (towers.length)
    towers.forEach(async (towerIndex) => {
      const images = await readdir(`assets/orig/${towerIndex}/`);

      if (images.length)
        images.forEach(async (image) => {
          const basename = extractBasename(image);

          await ensureDirectoryExistence(`assets/low/${towerIndex}/`);

          await sharp(`assets/orig/${towerIndex}/${image}`)
            .resize({ width: 2048, height: 1024 })
            .webp({ quality: 40 })
            .toFile(`assets/low/${towerIndex}/${basename}.webp`);

          await ensureDirectoryExistence(`assets/tiles/${towerIndex}/`);
          ...
          await sharp(`assets/orig/${towerIndex}/${image}`)
            ...
            .toFile(`assets/tiles/${towerIndex}/${basename}.dz`);

          await formatTilesDir(`assets/tiles/${towerIndex}`, basename);
        });
    });
}
